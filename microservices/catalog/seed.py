...
Images are shipped inside the container image (``catalog/seed_images``) and are
copied to ``catalog/static/uploads`` using their original file names, so every
replica of the catalog service serves the same image paths. The Dockerfile also
pre-populates ``static/uploads`` at build time, which means the paths recorded
here resolve on any pod, not just the one that ran the seed.
"""
import os
import shutil
...
SEED_IMAGES_DIR = os.environ.get(
...
def stage_images():
    """Copy the bundled seed images into the served uploads directory.

    Names are preserved so the resulting ``image_path`` values are identical on
    every replica. Safe to re-run — existing files are simply overwritten.
    """
    if not os.path.isdir(SEED_IMAGES_DIR):
        return
    os.makedirs(UPLOAD_DIR, exist_ok=True)
    for name in os.listdir(SEED_IMAGES_DIR):
        src = os.path.join(SEED_IMAGES_DIR, name)
...
def seed():
    with app.app_context():
        ...
        for p in PRODUCTS:
            product = dict(p)
            image_file = product.pop("image", None)
            if image_file:
                product["image_path"] = f"uploads/{image_file}"
